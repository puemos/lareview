  ...
  return (
    <TooltipPrimitive.Root delayDuration={0}>
      <TooltipPrimitive.Trigger asChild>
        <span className="bg-bg-tertiary text-text-tertiary hover:bg-bg-tertiary/70 cursor-help rounded-full px-2 py-0.5 text-[9px] font-semibold tracking-wide uppercase transition-colors">
          Rule
        </span>
          ...
          >
            ...
            {rule?.glob && (
              <div className="border-border/30 flex items-center gap-1.5 border-t pt-1.5">
                ...
                <code className="bg-bg-tertiary text-text-secondary rounded px-1 py-0.5 font-mono text-[9px]">
                  {rule.glob}
                </code>
              </div>
            )}
