//! UI Components

// Components are currently implemented inline in views.
// This module provides a namespace for reusable components.

pub mod diff_view;

// TODO: Extract reusable components:
// - header.rs — PR title, metadata
// - heatmap.rs — Risk visualization bar
// - sidebar.rs — Task navigation
// - task_card.rs — Collapsible task with diff, notes
// - diff_view.rs — Syntax-highlighted diff
// - note_input.rs — Text area
// - summary.rs — Review summary
// - agent_picker.rs — Select ACP agent
