...
pub fn reduce(state: &mut AppState, action: ReviewAction) -> Vec<Command> {
    match action {
        ...
        ReviewAction::SelectRun { run_id } => handle_select_run(state, run_id),
        ReviewAction::SelectTask { task_id } => select_task(state, task_id),
        ReviewAction::SelectTaskById { task_id } => handle_select_task_by_id(state, task_id),
        ...
        ReviewAction::DeleteReview(review_id) => handle_delete_review(state, review_id),
        ReviewAction::CreateFeedbackComment {
            ...
            title,
            body,
        } => handle_create_feedback_comment(
            state,
            task_id,
            feedback_id,
            file_path,
        ...
        ReviewAction::UpdateFeedbackStatus {
            feedback_id,
            status,
        } => handle_update_feedback_status(state, feedback_id, status),
        ReviewAction::UpdateFeedbackImpact {
            feedback_id,
            impact,
        } => handle_update_feedback_impact(state, feedback_id, impact),
        ReviewAction::UpdateFeedbackTitle { feedback_id, title } => {
            handle_update_feedback_title(state, feedback_id, title)
        }
        ...
        ReviewAction::RequestExportPreview => handle_request_export_preview(state),
        ...
        ReviewAction::ExportReviewToFile { path } => handle_export_review_to_file(state, path),
        ReviewAction::OpenInEditor {
            file_path,
            line_number,
        } => handle_open_in_editor(state, file_path, line_number),
        ReviewAction::ToggleExportOptionsMenu => {
            state.ui.show_export_options_menu = !state.ui.show_export_options_menu;
            Vec::new()
        }
        ReviewAction::SelectAllExportFeedbacks => handle_select_all_export_feedbacks(state),
        ReviewAction::ClearExportFeedbacks => handle_clear_export_feedbacks(state),
        ReviewAction::ToggleFeedbackSelection(feedback_id) => {
            handle_toggle_feedback_selection(state, feedback_id)
        }
        ReviewAction::UpdateExportOptions(options) => handle_update_export_options(state, options),
        ...
        ReviewAction::ConfirmSendToPr => handle_confirm_send_to_pr(state),
    }
}

// --- Action Handlers ---

fn handle_navigate_to_feedback(
    state: &mut AppState,
...
fn handle_refresh_github_review(state: &mut AppState) -> Vec<Command> {
...
fn handle_select_review(state: &mut AppState, review_id: String) -> Vec<Command> {
...
fn handle_select_run(state: &mut AppState, run_id: String) -> Vec<Command> {
...
fn handle_select_task_by_id(state: &mut AppState, task_id: TaskId) -> Vec<Command> {
...
fn handle_delete_review(state: &mut AppState, review_id: String) -> Vec<Command> {
    state.ui.review_error = None;
...
fn handle_create_feedback_comment(
    state: &mut AppState,
    task_id: TaskId,
...
fn handle_update_feedback_status(
    state: &mut AppState,
    feedback_id: String,
...
fn handle_update_feedback_impact(
    state: &mut AppState,
    feedback_id: String,
...
fn handle_update_feedback_title(
    state: &mut AppState,
    feedback_id: String,
    title: String,
) -> Vec<Command> {
...
fn handle_request_export_preview(state: &mut AppState) -> Vec<Command> {
...
fn handle_export_review_to_file(state: &mut AppState, path: std::path::PathBuf) -> Vec<Command> {
    if let (Some(review_id), Some(run_id)) = (
        state.ui.selected_review_id.as_ref(),
        state.ui.selected_run_id.as_ref(),
    ) {
        if let Some(crate::ui::app::OverlayState::Export(ref mut data)) = state.ui.active_overlay {
            data.is_exporting = true;
...
fn handle_open_in_editor(
    ...
    let Some(request) = resolve_editor_open_request(state, &file_path, line_number) else {
        state.ui.review_error = Some("Link a repository to open files in an editor.".to_string());
        return Vec::new();
    };
...
fn handle_select_all_export_feedbacks(state: &mut AppState) -> Vec<Command> {
    if let Some(crate::ui::app::OverlayState::Export(ref mut data)) = state.ui.active_overlay {
...
fn handle_clear_export_feedbacks(state: &mut AppState) -> Vec<Command> {
    if let Some(crate::ui::app::OverlayState::Export(ref mut data)) = state.ui.active_overlay {
...
fn handle_toggle_feedback_selection(state: &mut AppState, feedback_id: String) -> Vec<Command> {
    if let Some(crate::ui::app::OverlayState::Export(ref mut data)) = state.ui.active_overlay {
...
fn handle_update_export_options(
    state: &mut AppState,
    options: crate::ui::app::state::ExportOptions,
) -> Vec<Command> {
    if let Some(crate::ui::app::OverlayState::Export(ref mut data)) = state.ui.active_overlay {
...
fn handle_confirm_send_to_pr(state: &mut AppState) -> Vec<Command> {
    let Some(review_id) = state.ui.selected_review_id.clone() else {
        if let Some(crate::ui::app::OverlayState::SendToPr(ref mut data)) = state.ui.active_overlay
        {
