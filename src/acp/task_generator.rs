...
fn build_prompt(pr: &PullRequest, diff_text: &str) -> String {
    format!(
        r#"You are an expert code review assistant. Your job is to read the pull request diff and create a small set of clear, actionable review tasks for the author and reviewers, organized by logical sub-flows in the change.
...
- patches:
  - A list of hunks that show the most relevant parts of the diff for this task.
  - For each patch:
    - file: The file path.
    - hunk: A slice of the unified diff for that file that is relevant to this sub-flow.
  - Collect all hunks that are central to this flow, even if they are in different files.
  - Include enough context to understand the change, but do not include the entire file.
  - Avoid duplicating the same hunk across multiple tasks unless it genuinely belongs to more than one flow.
